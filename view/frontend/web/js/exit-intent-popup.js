define([
  ...
  return Component.extend({
    ...
    _removeCouponIfApplied: function () {
      ...
      storage
        .delete(
          urlBuilder.createUrl("/guest-carts/:cartId/coupons", {
            cartId: quoteId,
          }),
        )
        .done(function () {
        ...
        .fail(function () {
          /* fail-open — server backstop will enforce */
        });
    },
    ...
    _shownThisSession: function () {
      return (
        this.config.popupFrequency !== "always" &&
    ...
    _loginPromptShownThisSession: function () {
      return (
        this.config.loginPrompt.frequency !== "always" &&
    ...
    _markLoginPromptShownThisSession: function () {
      ...
      sessionStorage.setItem(
        loginPromptSessionShownKey(this.config.ruleId),
        "1",
      );
    },
    ...
    _attachDesktopListeners: function () {
      var self = this;
      var onExitCheck = throttle(function (e) {
        if (e.clientY <= 5) {
          self._trigger();
        }
      }, THROTTLE_MS);

      // mousemove catches the common case reliably across all browsers.
      document.addEventListener("mousemove", onExitCheck);

      // mouseout + relatedTarget === null catches the mouse actually
      // leaving the browser window — needed because mouseleave alone
      // is unreliable in Firefox/Safari for this scenario.
